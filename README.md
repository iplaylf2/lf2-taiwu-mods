...
> **理想与现实**
> 理想情况下，`dotnet restore` 能一键拉取所有依赖。但由于分发游戏文件存在法律风险，我们不提供游戏核心库的在线包。因此，在首次运行 `dotnet restore` 时，你可能会遇到 `NU1101` 等包找不到的错误。要解决此问题，你需要手动构建这些缺失的包。相关说明请参阅本文档的[管理非托管依赖](#-管理非托管依赖)章节。
...
本模板所需的部分游戏核心库与第三方库未发布至官方 NuGet 源，需要手动打包以供本地恢复。这是解决“快速上手”章节中提到的 `dotnet restore` 失败问题的推荐方法。

关于此操作的详细步骤、设计缘由、以及为何我们选择本地恢复而非提供在线包，请参阅其 [**详细文档**](./projects/unmanaged-vendor/README.md)。
