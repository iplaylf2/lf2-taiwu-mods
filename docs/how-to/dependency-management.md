...
关于游戏官方程序集的打包细节，已整理为专门的《[游戏依赖打包参考手册](../reference/game-libs-packaging.md)》。如果需要准备《太吾绘卷》官方 DLL，请优先阅读该参考文档。
...
> [!TIP]
> 为了避免不同 Mod 之间因携带不同版本的库而产生冲突，强烈建议使用 `ILRepack` 将这类运行时依赖内嵌到你的主程序集中。需要说明的是，这种内嵌处理在 `Release` 构建模式下是默认行为。
>
> `ILRepack` 是一个程序集合并工具，能够将多个 DLL 文件合并为单个程序集，从而解决依赖冲突问题。[详细了解 ILRepack 和其他构建工具](../reference/build-system.md#核心工具链介绍)

## 通过物料覆盖进行深度定制

> **注意**：本节内容与 `projects/unmanaged-vendor/README.md` 中描述的自动化工作流密切相关，建议先了解该工作流的基本原理。

在自动化工作流执行 `dotnet pack projects/unmanaged-vendor/game/game.slnx` 之前，系统会先将用户提供的游戏库压缩包解压并覆盖到 `projects/unmanaged-vendor/game` 目录。这个"覆盖"行为实际上是一个强大的干预点，可供高级用户灵活利用。
